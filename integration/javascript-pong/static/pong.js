...
Pong = {
  ...
  getPredictUrl: function() {
    return Pong.PredictUrl;
  },

  start: function(numPlayers) {
    if (!this.playing) {
      if (numPlayers == 1) {
        Pong.PredictUrl = document.getElementById('select-mode').value;
        
        if (Pong.PredictUrl == 'unselected') {
          window.alert('Please select a single player mode from the drop down menu on the left!')
          return;
        }
      } else {
        Pong.PredictUrl = null;
      }
  ...
  goal: function(playerNo) {
    ...
    if (this.scores[playerNo] == 5) {
      ...
      if (Pong.PredictUrl != null) {
        ...
        var obj = {'human': this.scores[1]};
        obj[Pong.PredictUrl] = this.scores[0];
  ...
  Paddle: {
    ...
    ai: function(dt, ball) {
      ...
      fetch('http://127.0.0.1:1337/pong/predict/', {
        method: 'POST',
        redirect: 'follow',
