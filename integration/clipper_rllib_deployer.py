...
from clipper_admin.deployers.deployer_utils import save_python_function, serialize_object
...
def create_endpoint(clipper_conn,
    ...
    clipper_conn.register_application(name, input_type, default_output,
                                      slo_micros)
    deploy_rllib_model(clipper_conn, name, version, input_type, func,
...
def deploy_rllib_model(clipper_conn,
                         name,
                         version,
                         input_type,
                         func,
    ...
    try:
        ...
        # copy checkpoint_dir into serialization_dir/checkpoint
        ...
        if base_image == "default":
            if py_minor_version < (3, 0):
                ...
                base_image = "{}/rllib-container:{}".format(
                    __registry__, __version__)
            elif py_minor_version == (3, 5):
                logger.info("Using Python 3.5 base image")
                base_image = "hsubbaraj/rllib35-container:version1"
                # base_image = "{}/rllib35-container:{}".format(
                #     __registry__, __version__)
            elif py_minor_version == (3, 6):
                logger.info("Using Python 3.6 base image")
                base_image = "{}/rllib36-container:{}".format(
                    __registry__, __version__)
            else:
                msg = (
                    "RLlib deployer only supports Python 2.7, 3.5, and 3.6. "
        ...
        raise ClipperException("Error saving RLlib model: %s" % e)
